...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand-title { font-size: 20px; font-weight: bold; color: <?= esc($primaryColor ?? '#0284c7') ?>; letter-spacing: -0.5px; }
        .brand-subtitle { font-size: 10px; color: #64748b; margin-top: 2px; }
        .doc-title { font-size: 22px; font-weight: 800; color: <?= esc($primaryColor ?? '#0284c7') ?>; text-transform: uppercase; letter-spacing: 1px; text-align: right; }
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td>
                ...
                <div class="brand-subtitle"><?= esc($company['tagline'] ?? ($company['address'] ?? 'Human Resources & Payroll Dept')) ?></div>
            </td>
            <td>
                <div class="doc-title">PAYSLIP</div>
                <div class="doc-meta">Period: <strong><?= esc($payPeriod ?? date('F Y')) ?></strong></div>
                <div class="doc-meta">Pay Date: <?= esc($payDate ?? date($dateFormat ?? 'M d, Y')) ?></div>
    ...
    <div class="footer">
        ...
        <?php else: ?>
            <div><?= esc($company['name'] ?? 'Company') ?> &bull; Computer-generated payslip, requires no physical signature.</div>
        <?php endif; ?>
        <?php if (!empty($showPoweredBy)): ?>
            <div style="margin-top: 3px; font-size: 7.5px; color: #cbd5e1;">Generated by Jengo PDF Payroll Engine</div>
        <?php endif; ?>
    </div>
