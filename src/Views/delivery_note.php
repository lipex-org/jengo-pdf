...
<head>
    <meta charset="UTF-8">
    <title>Delivery Note <?= esc($deliveryNumber ?? 'DN-2026-001') ?></title>
    <style>
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td>
                <div class="brand-title"><?= esc($company['name'] ?? 'Company / Logistics') ?></div>
                <div class="brand-subtitle"><?= esc($company['tagline'] ?? 'Dispatch & Warehousing Operations') ?></div>
            </td>
            <td>
                <div class="doc-title">DELIVERY NOTE</div>
                <div class="doc-meta">DN #: <strong><?= esc($deliveryNumber ?? 'DN-2026-8802') ?></strong></div>
                <div class="doc-meta">Date: <?= esc($dispatchDate ?? date('M d, Y')) ?></div>
                <div class="doc-meta">PO Ref: <?= esc($poNumber ?? 'PO-99120') ?></div>
            </td>
        </tr>
    </table>
    ...
    <table class="info-table">
        <tr>
            <td>
                ...
                <div class="entity-name"><?= esc($recipient['name'] ?? 'Receiving Company / Client') ?></div>
    ...
    <div class="logistics-card">
        <table class="logistics-grid">
            <tr>
                ...
                <td class="log-val"><?= esc($trackingNumber ?? 'TRK-9812401824') ?></td>
            </tr>
            <tr>
                <td class="log-key">Vehicle Reg:</td>
                <td class="log-val"><?= esc($vehicleNumber ?? 'KBZ 892M') ?></td>
                <td class="log-key">Package Count & Weight:</td>
                <td class="log-val"><?= esc($totalPackages ?? '4 Boxes') ?> &bull; <?= esc($totalWeight ?? '18.5 kg') ?></td>
            </tr>
        </table>
    </div>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center" style="font-weight: bold; color: #0284c7;"><?= esc($item['shipped_qty'] ?? 1) ?> <?= esc($item['unit'] ?? 'units') ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <div class="footer">
        Generated by Jengo PDF Engine &bull; Official Dispatch Manifest
    </div>
