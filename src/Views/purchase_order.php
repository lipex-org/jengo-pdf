...
<head>
    <meta charset="UTF-8">
    <title>Purchase Order <?= esc($poNumber ?? 'PO-2026-001') ?></title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .po-title { font-size: 24px; font-weight: 800; color: #0f172a; text-transform: uppercase; letter-spacing: 1px; text-align: right; }
        .po-meta { font-size: 10px; color: #64748b; text-align: right; margin-top: 3px; }
        ...
        .terms-grid-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px 14px; margin-bottom: 20px; }
        .terms-grid { width: 100%; border-collapse: collapse; }
        .terms-grid td { padding: 4px 6px; font-size: 10px; }
        .terms-key { color: #64748b; }
        .terms-val { color: #0f172a; font-weight: 600; }
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td>
                ...
                <?php endif; ?>
                <div class="brand-title"><?= esc($company['name'] ?? 'Purchasing Organization') ?></div>
                <div class="brand-subtitle"><?= esc($company['tagline'] ?? 'Procurement & Supply Chain Management') ?></div>
            </td>
            <td>
                <div class="po-title">PURCHASE ORDER</div>
                <div class="po-meta">PO #: <strong><?= esc($poNumber ?? 'PO-2026-0814') ?></strong></div>
                <div class="po-meta">Date: <?= esc($poDate ?? date($dateFormat ?? 'M d, Y')) ?></div>
                <div class="po-meta">Delivery Date: <?= esc($expectedDate ?? date($dateFormat ?? 'M d, Y', strtotime('+14 days'))) ?></div>
            </td>
        </tr>
    </table>
    ...
    <table class="info-table">
        <tr>
            <td>
                ...
                <div class="entity-name"><?= esc($vendor['name'] ?? 'Vendor Name') ?></div>
                <?php if (!empty($vendor['contact'])): ?><div class="entity-detail">Attn: <?= esc($vendor['contact']) ?></div><?php endif; ?>
                <?php if (!empty($vendor['address'])): ?><div class="entity-detail"><?= esc($vendor['address']) ?></div><?php endif; ?>
                <?php if (!empty($vendor['email'])): ?><div class="entity-detail"><?= esc($vendor['email']) ?></div><?php endif; ?>
            </td>
            <td style="text-align: right;">
                <div class="section-label">Deliver / Ship To:</div>
                <div class="entity-name"><?= esc($shipTo['name'] ?? ($company['name'] ?? 'Company Name')) ?></div>
                <?php if (!empty($shipTo['address'])): ?><div class="entity-detail"><?= esc($shipTo['address']) ?></div><?php endif; ?>
                <?php if (!empty($shipTo['contact'])): ?><div class="entity-detail">Receiving Dept: <?= esc($shipTo['contact']) ?></div><?php endif; ?>
                <?php if (!empty($shipTo['phone'])): ?><div class="entity-detail">Phone: <?= esc($shipTo['phone']) ?></div><?php endif; ?>
            </td>
        </tr>
    </table>

    <div class="terms-grid-box">
        <table class="terms-grid">
            <tr>
                <td class="terms-key">Payment Terms:</td>
                <td class="terms-val"><?= esc($paymentTerms ?? 'Net 30 Days') ?></td>
                <td class="terms-key" style="text-align: right;">Shipping Method / FOB:</td>
                <td class="terms-val" style="text-align: right;"><?= esc($shippingMethod ?? 'Courier / Destination') ?></td>
            </tr>
        </table>
    </div>
    ...
    <table class="items-table">
        <thead>
            <tr>
                <th style="width: 6%;">#</th>
                <th style="width: 18%;">Part / Item #</th>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $idx + 1 ?></td>
                <td style="font-family: monospace; font-size: 9px; color: #64748b;"><?= esc($item['sku'] ?? 'PART-' . ($idx+1)) ?></td>
                <td>
        ...
        <tr>
            <td style="width: 55%;">
                <div class="notes-card">
                    <div style="font-weight: bold; color: #0f172a; margin-bottom: 4px; text-transform: uppercase;">Special Instructions</div>
                    <p><?= esc($instructions ?? 'Please include PO number on all invoices and packing slips. Deliveries accepted Mon-Fri 8am-4pm.') ?></p>
                </div>
            </td>
            <td style="width: 45%;">
                <table class="calculation-table">
                    ...
                    <?php if ($shipping > 0): ?>
                    <tr>
                        <td class="calc-label">Freight / Shipping:</td>
                        <td class="calc-value"><?= \Jengo\Pdf\Support\Currency::format($shipping, $currency ?? '$') ?></td>
                    </tr>
                    <?php endif; ?>
                    <?php if ($taxRate > 0): ?>
                    <tr>
                    ...
                    <tr class="calc-grand-total">
                        <td class="calc-label grand-total-label">PO Total:</td>
    ...
    <table class="auth-table">
        <tr>
            <td>
                <div class="sign-line">Authorized Purchasing Officer</div>
            </td>
            <td style="text-align: right;">
                <div class="sign-line" style="margin-left: auto;">Vendor Confirmation & Acceptance</div>
            </td>
        </tr>
    </table>
    ...
    <div class="footer">
        ...
        <?php else: ?>
            <div><?= esc($company['name'] ?? 'Company') ?> &bull; Official Procurement Document</div>
        <?php endif; ?>
